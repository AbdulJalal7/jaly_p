...
const challengesApi = {
    ...
    try {
      ...
      const populated = await Promise.all(res.documents.map(async (doc) => {
        try {
          ...
          return { ...doc, creator_name: userDoc.name || "Unknown", creator_avatar: userDoc.avatar || null };
        } catch (e) {
          return { ...doc, creator_name: "Unknown", creator_avatar: null };
        }
      }));
    ...
    try {
      ...
      const populated = await Promise.all(parts.documents.map(async (p) => {
        try {
          ...
          return { ...p, user_name: userDoc.name || "Unknown Player", user_avatar: userDoc.avatar || null };
        } catch (e) {
          return { ...p, user_name: "Unknown Player", user_avatar: null };
        }
      }));
   ...
   */
  async uploadMatchResult(challengeId, userId, stringFileUrl, outcome) {
    try {
      ...
      if (existing.total > 0) {
        ...
        const updateData = isChallenger 
          ? { challenger_screenshot: stringFileUrl, challenger_outcome: outcome }
          : { opponent_screenshot: stringFileUrl, opponent_outcome: outcome };
        ...
        if (isChallenger) {
          ...
          createData.challenger_outcome = outcome;
        } else {
          createData.opponent_screenshot = stringFileUrl;
          createData.opponent_outcome = outcome;
        }
