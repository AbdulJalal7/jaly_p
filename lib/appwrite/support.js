...
class SupportService {
  ...
  // 9. Get or create a persistent active ticket for the user
  async getOrCreateActiveTicket(userId) {
    try {
      // Fetch the most recent ticket for the user
      const tickets = await this.getUserTickets(userId);
      
      if (tickets.total > 0) {
        // Return the most recent ticket
        return tickets.documents[0];
      }
      ...
      return await this.createTicket({
        ...
        message: "Welcome to our support center. How can we help you today?"
      });
    } catch (error) {
      console.error("Error in getOrCreateActiveTicket:", error);
      throw error;
    }
  }
}
