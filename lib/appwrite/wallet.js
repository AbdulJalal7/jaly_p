...
class WalletService {
  ...
  async approveDeposit(transactionId, internalUserId, amount) {
    try {
      ...
      console.log("transactionId",transactionId, "internalUserId",internalUserId.name, "amount",amount,"current balance",internalUserId.wallet_balance,"current total deposit",internalUserId.total_deposit);
      ...
      const currentBalance =  internalUserId.wallet_balance || 0;
      const currentTotalDeposit = internalUserId.total_deposit || 0;
  ...
  async approveWithdrawal(requestId, internalUserId, amount) {
    try {
      ...
      const currentBalance = internalUserId.wallet_balance || 0;
      const currentTotalWithdraw = internalUserId.total_withdraw || 0;
