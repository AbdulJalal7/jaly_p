...
export const AuthProvider = ({ children }) => {
  ...
  useEffect(() => {
    ...
    // Initial subscription when component mounts or user changes
    subscribeToRealtime();
    ...
    const appStateSubscription = AppState.addEventListener("change", (nextAppState) => {
      if (nextAppState === "active") {
        ...
        subscribeToRealtime();
        // Since we might have missed events while disconnected, refresh the user
        restoreSession(); 
      } else if (nextAppState === "background" || nextAppState === "inactive") {
        // App has gone to the background, disconnect gracefully
        cleanupSubscription();
      }
    });
  ...
  // Called after a successful Google OAuth — updates the context state
  // and persists the session to SecureStore just like a normal login.
  const loginWithGoogle = async () => {
    const currentUser = await authService.getCurrentUser();
    setUser(currentUser);
    savePushToken(currentUser.$id);
    await SecureStore.setItemAsync(STORAGE_KEY, JSON.stringify(currentUser));
    return currentUser;
  };
